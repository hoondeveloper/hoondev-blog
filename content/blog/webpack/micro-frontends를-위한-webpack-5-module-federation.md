...
싱글 페이지 애플리케이션에 사용되는 페이지들을 각각의 분리된 빌드로 나누고, Host 빌드에서 그 페이지들을 동적으로 불러와 렌더링한다.

이 방식을 취하면

- 각 페이지들은 모두 각각 개발되어 배포한다. 한 페이지의 코드가 변경되었다고 모든 페이지를 빌드할 필요가 없다.
- Host 앱은 라우팅이 변경되거나 추가될 때 등 제한적인 상황에만 배포하면 된다.
...
![실행 화면](./images/module_federation_serve.png)
